            ...
            allQuestionsData[testId].forEach(q => {
                if (q.type !== 'header') {
                    ...
                    if (q.type === 'open_ended' && q.model_answer && typeof q.model_answer === 'object') {
                        answerText = q.model_answer.summary + ' ' + (q.model_answer.keywords ? q.model_answer.keywords.map(kw => kw.keyword + ' ' + kw.explanation).join(' ') : '');
                    } else {
    ...
    function renderQuestions() {
        ...
        currentTestQuestions.forEach((q, index) => {
            ...
            const answerText = q.type === 'open_ended' ? 
                (q.model_answer ? q.model_answer.summary : '') : 
                (q.answer || '');
            
            helpButtonsHTML += `
                <button type="button" class="help-btn show-answer-btn" data-answer="${answerText}">
                    <i class="bi bi-check-circle-fill" style="font-size: 24px; color: var(--success-color);"></i>
                </button>`;
